	...
	Для Каждого ФайлКЗагрузке Из ФайлыВКаталоге Цикл  
		...
		Попытка  
			...
			КонтекстЧтения = Новый Структура(
				"ТекстОшибки, Отказ, НастройкиЧтения, ДанныеJSON, НаборЗаписей",
			    "", 
				Ложь, 
				НастройкаТекущегоФайла, 
				Неопределено,
				НЗДанныеСОборудования
			);
...
Процедура ДобавитьСтрокуВРСДанныеСОборудования()
...
Функция ПолучитьПустуюСтруктуруЗаполненияСтрокиРСДанныеСОборудования()
	...
	Возврат Новый Структура("ИмяФайла, ПорядковыйНомерСтроки, ДанныеСтрокиJSON, ДатаОперации, ТипОтчета");
...
Функция ПолучитьЗначениеЯчейки(ТабДок, Стр, Кол) 
